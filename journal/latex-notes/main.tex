...
\subsection{Empirical bservations}
\begin{itemize}
...
\begin{align*}
    ...
    \text{let}\quad r_{t}^\theta:= r_{t}^\theta((1-s)r_{t} + s\epsilon) &= r_{t}^\theta(z_{t}) \\
    \text{s.t.} \quad \epsilon_{t}^\theta &= \frac{z_{t} - (1-s)r_{t}^\theta}{s} \\
    \text{then} \quad  u_{t}^\theta =  r_{t}^\theta - \epsilon_{t}^\theta &= \frac {s}{s}r_{t}^\theta - \frac{z_{t} - (1-s)r_{t}^\theta}{s} \\
    &= \frac{r_{t}^\theta-z_{t}}{s} 
\end{align*}
where, during the generation process, $z_{t}=r_{t-\delta}^\theta$ is the noisy state at the previous time step (or pure noise at initialization of sampling, to confirm plug in $s=0$ and $s=1$), and $s=\frac{t}{T}$ (progressing from 0 to 1, although in their implementation they do the opposite, and go from 1 to 0).
...
\subsection{Loss}
At time $t$, given a scalar value $y_t$ (in normalized space) denoting the guidance term, we define the squared l2 distance from the denoised state $z_t$ as follows:
\begin{align*} 
    \mathcal{L}(y_t, z_t) = \frac{1}{2} \left\lVert y_t - \frac{\sum_i m_i \, (x_t^i + \sigma^i z_t^i)}{\sum_i m_i}   \right\lVert ^2
\end{align*}
Note that we the define guidance $y_t$ in the denormalized space.

We are then interested in the gradient $\nabla_{z_t}\mathcal{L}(y_t, z_t)$, which we can compute in closed form:
\begin{align*} 
    \text{let} \quad \hat{m}(z_t) &:= \frac{\sum_i m_i \, (x_t^i + \sigma^i z_t^i)}{\sum_i m_i} \\
    \text{then} \quad \nabla_{z_t}\mathcal{L}(y_t, z_t) &= \nabla_{z_t}\frac{1}{2} \left\lVert y_t - \hat{m}(z_t) \right\lVert ^2 \\
    &= \nabla_{z_t}\frac{1}{2} \left(\hat{m}(z_t)^2 - 2 \,y_t \, \hat{m}(z_t) \right) \\
    &= \hat{m}(z_t) \nabla_{z_t} \hat{m}(z_t) - y_t \nabla_{z_t} \hat{m}(z_t) 
\end{align*}
From this we extract the partial derivatives and stack them to put together the final gradient vector:
\begin{align*}
     \frac{\partial \, \hat{m}(z_t)}{z_t^j} &= \frac{m^i\sigma^i}{\sum_i m_i} \\
    \text{then} \quad \frac{\partial \,  \mathcal{L}(y_t, z_t)}{z_t^j} &= \hat{m}(z_t) \frac{m^i\sigma^i}{\sum_i m_i} - y_t \frac{m^i\sigma^i}{\sum_i m_i}
\end{align*}
For practical reasons we use automatic differentiation, but this might be worthwhile for future usage.

\subsection{Computing the loss in denormalized space}
Unfortunately, we cannot define $y_t$ in normalized space directly and have a direct translation to denormalized space. We need instead to define $y_t$ in denormalized space, and denormalize $\hat{r}_t$ instead.
...
\begin{align*}
    \tilde{u}_t(z|y) = u_t(z) - \lambda_t \nabla_{z}\mathcal{L}(y, z^\theta(z_t)) 
\end{align*}
Note that with $w\neq 1$ we abandon the vector field $u(x|y)$ in favor of $\tilde{u}_t(z|y)$, losing the original probabilistic interpretation (still in the spirit of classifier guidance).
...
\begin{align*}
    \bar{u}_t(z|y) &:= (1-w)u_t(z) - (1-w)\lambda_t \nabla_{z}\mathcal{L}(y, z^\theta(z_t)) \\
    \text{or maybe even} \quad \bar{u}_t(z|y) &:= (1-w_t)u_t(z) - (1-w_t) \nabla_{z}\mathcal{L}(y, z^\theta(z_t)) 
\end{align*}
