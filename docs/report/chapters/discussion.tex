...
% \subsection{Supporting diagnostic: intermediate prediction error}
% \label{sec:evaluation-prediction-noise}

% To investigate whether guidance effectiveness is associated with the accuracy
% of the intermediate clean prediction, we additionally inspect the unguided
% residual flow.

% At flow step $t$, the clean estimate of the final latent is
...
%         z_{t+1}-z_t
%     \right).
%     \label{eq:intermediate-latent-prediction}
% \end{equation}

% Its error relative to the realized final latent $z_T$ is measured over the
% evaluation region by
...
%         \right\rangle_{\pi}
%     }.
%     \label{eq:intermediate-prediction-rmse}
% \end{equation}

% This diagnostic is not used for profile ranking. It is included only to
% interpret the temporal location at which guidance is found to be most
% effective.
