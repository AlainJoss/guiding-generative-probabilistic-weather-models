...
interest through a mask $\pi$ over variable, vertical level, and space.

Let $v^\star$ denote the target variable and
$\ell^\star\in\mathcal{L}_{v^\star}$ its target level. Furthermore, let
\[
    k_{ij}\in\{0,1\}
\]
...
The mask is non-zero only for the selected variable and level and within the
selected region. To obtain a regional average that accounts for differences in
the physical area represented by each grid cell, we weight each included
latitude row according to
\begin{equation}
    a_i
    =
    \int_{\beta_i^-}^{\beta_i^+}
...
Following the convention introduced in \Cref{sec:archesweather-dataset},
the inverse normalization required to evaluate this diagnostic in physical
units is implicit in $\mathcal{M}_\pi$.

For the experiments in this thesis, the target variable is temperature and the
target level is the surface,
\[
    v^\star = \mathrm{temperature},
    \qquad
    \ell^\star = 0,
\]
such that $\mathcal{M}_\pi(x_n)$ represents the
area-weighted mean $2$~m temperature within the selected target region.
...
\begin{figure}[t]
    ...
    \begin{subfigure}[c]{0.48\textwidth}
        \centering
        \includegraphics[height=5.cm]{figures/mask_globe.png}
