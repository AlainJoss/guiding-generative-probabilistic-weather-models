...
\section{Target variable, level, and region}
\label{sec:target-region}

Using the state representation introduced in
\Cref{sec:archesweather-dataset}, we specify the atmospheric quantity of
interest through a target variable $v^\star$, target level
$\ell^\star\in L_{v^\star}$, and a spatial mask $\pi$.
...
To account for differences in the physical area represented by each grid
cell, we weight each included latitude row according to
...
We define the normalized spatial mask as
...
For any atmospheric field $x_{v\ell}$, the corresponding spatial
masked-average operator is

\begin{equation}
    \mathcal{M}_{\pi}(x_{v\ell})
    :=
    \sum_{i,j}
    \pi_{ij}x_{v\ell ij}.
...
Following the convention introduced in
\Cref{sec:archesweather-dataset}, the inverse normalization required to
evaluate this diagnostic in physical units is implicit in
$\mathcal{M}_{\pi}$.

For the experiments in this thesis, the target variable and level are

\[
    v^\star=\mathrm{temperature},
    \qquad
    \ell^\star=0,
\]
...
\[
    \mathcal{M}_{\pi}(x_{v^\star\ell^\star})
\]

represents the area-weighted mean 2\,m temperature within the selected target
region.
...
example target region over Europe. Its support is rectangular on the
...
\begin{figure}[t]
    ...
    \begin{subfigure}[c]{0.48\textwidth}
        \centering
        \includegraphics[height=5.1cm]{figures/mask_flat.png}
    ...
    \begin{subfigure}[c]{0.48\textwidth}
        \centering
        \includegraphics[height=5.0cm]{figures/mask_globe.png}
        \caption{Spherical representation.}
        \label{fig:target-mask-globe}
    \end{subfigure}
    \caption{
        Example spatial target mask. The rectangular support determines
...
Normalizing $\pi$ to sum to one ensures that
$\mathcal{M}_{\pi}$ remains an average and is therefore directly comparable
...
TODO: need to rewrite from now until chapter 4 included all notation regarding the averaging operator.
For instance
$$
q_{n,m}^{\mathrm{ung}}
:=
\mathcal{M}_{\pi}
\left(
    x_{n,m,v^\star\ell^\star}^{\mathrm{ung}}
\right)
$$
...
Before generating a guided ensemble, we first generate an unguided reference
ensemble of $M$ trajectories over the experimental horizon $n=1,\ldots,N$,
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{figures/rollouts.png}
    \caption{
